...
DB_NAME="fidranet_db"
DB_USER="F1dranet@dmin_user"
DB_PASS="F1dranetP@ssMss@P_user"

RED="\e[31m"; GREEN="\e[32m"; YELLOW="\e[33m"; CYAN="\e[36m"; RESET="\e[0m"

echo -e "
${CYAN}----------------------------------------------${RESET}"
echo -e "${CYAN}  Fidranet PAM Installation Script"
echo -e "----------------------------------------------${RESET}
"
...
echo -e "${CYAN}4. Download Fidranet War And Install...${RESET}"
...
echo -e "${GREEN}fidranet war file setup completed successfully.${RESET}\n"

# --------- Guacamole Database Setup And Guacamole Properties. ----------
...
GUAC_VERSION="1.6.0"
JDBC_VERSION="8.0.30"
JDBC_DL_URL="https://repo1.maven.org/maven2/mysql/mysql-connector-java/$JDBC_VERSION/mysql-connector-java-$JDBC_VERSION.jar"
...
if [ -d "$TOMCAT_INSTALL_DIR/webapps/guacamole/WEB-INF/lib" ]; then
    if [ ! -f "$JDBC_TARGET_WEBINF" ]; then
        ...
        echo -e "${GREEN}MySQL JDBC driver copied to WEB-INF/lib.${RESET}"
    else
        echo -e "${GREEN}MySQL JDBC driver already present in WEB-INF/lib.${RESET}"
    fi
else
